...
<div class="row">
    <form action="" method="POST" style="align-content: center">
            <input type="text" id="booksearch" name="booksearch" placeholder="Enter Book Title"/><br>
...
{
    $conn = makeDBconnection();
    $sqlbest = "SELECT distinct B.Title, A.First_Name, A.Last_Name " .
        "FROM bookstore.books AS B " .
        "JOIN bookstore.author AS A ON A.AuthID = B.AuthID " .
...
if(isset($_POST['cCart']))
{
    ...
    $conn = makeDBconnection();
    $sql = "INSERT INTO bookstore.cart".
            "VALUES ('Customer1', \"{$_POST['BookIDValue']}\")";
    mysqli_query($conn, $sql);
}
elseif(isset($_Post['cWish']))
{
    echo("<p>{$_POST['cWish']} has been updated</p>");
    $conn = makeDBconnection();
    $sql = "INSERT INTO bookstore.wishlist".
            "VALUES ('Customer1', \"{$_POST['BookIDValue']}\")";
    mysqli_query($conn, $sql);
}
